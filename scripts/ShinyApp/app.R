...
# ── Color palette — fixed to global range across all years ────────────────────
global_range <- range(county_lead$Rate_per_1000, na.rm = TRUE)
global_pal   <- colorNumeric(
...
ui <- fluidPage(
  tags$head(tags$style(HTML("
    ...
    * { box-sizing: border-box; }
    ...
    /* ── Fix: hide minor tick marks that don't align to whole years ── */
    .irs--shiny .irs-grid-pol.small { display: none; }
    ...
    .slider-animate-button i { font-size: 16px !important; }
    ...
    .map-legend-bar .legend-gradient {
      flex: 1;
      height: 14px;
      border-radius: 4px;
      background: linear-gradient(to right,
        #ffffff, #ffe5d9, #fcbba1, #fc9272,
        #fb6a4a, #de2d26, #a50f15);
      border: 1px solid #ddd0c8;
    }
  ...
  div(class = "main-panel",
      ...
      div(class = "scroll-nudge",
          "Scroll down for background, data sources, and disclosures",
      ...
      div(class = "map-legend-bar",
          span("Low rate", style = "white-space: nowrap;"),
          div(class = "legend-gradient"),
          span("High rate", style = "white-space: nowrap;"),
          span("·", style = "color: #ccc; margin: 0 4px;"),
          span("Gray = no data for selected year",
               style = "white-space: nowrap; color: #999;")
      ),
      ...
      div(class = "about-section",
          tags$button(
            class = "about-toggle", id = "about_btn",
            onclick = "
      ...
      div(class = "about-section",
          tags$button(
            class = "about-toggle", id = "data_btn",
            onclick = "
        ...
        ",
          ...
          div(id = "data_body", class = "about-body",
              HTML("
          <p>All information analyzed was from open source datasets. You can
          access the following data with definitions and codebooks here:</p>
          <ul>
            <li><strong>Primary dataset:</strong> New York State Department of
            Health — Childhood Blood Lead Testing and Elevated Incidence by ZIP
            Code, Beginning 2000<br>
            <a href='https://health.data.ny.gov/Health/Childhood-Blood-Lead-Testing-and-Elevated-Incidenc/d54z-enu8/about_data'
               target='_blank'>health.data.ny.gov</a></li>
            <li><strong>FIPS crosswalk:</strong> New York State ZIP Codes —
            County FIPS Cross-Reference<br>
            <a href='https://data.ny.gov'
               target='_blank'>data.ny.gov</a></li>
          </ul>
        ")
          )
      ),
      ...
      div(class = "footer",
          div(class = "footer-map-note",
              "Click any county on the map to see details for the selected year."
          ),
          div(class = "footer-meta",
              tags$span(
                "Cam Lincoln, MPH '26 (cal386@cornell.edu)",
                style = "color: #2b1f1f; font-weight: 600;"
              ),
              tags$span(" · ", style = "color: #ccc;"),
              tags$a("GitHub: cal386",
                     href   = "https://github.com/cal386/VTPEH-repo",
                     target = "_blank",
                     style  = "color: #a50f15; text-decoration: none; font-weight: 500;")
          )
      )
  )
)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    leafletProxy("map", data = data) %>%
      ...
      clearControls() %>%
      addPolygons(
        fillColor    = ~global_pal(Rate_per_1000),
        fillOpacity  = 0.82,
        color        = "#6b4a4a",
        weight       = 0.8,
        opacity      = 0.6,
  ...
  output$county_info <- renderUI({
    ...
    HTML(paste0(
      "<span class='info-label'>County:</span><br>", county, "<br><br>",
      "<span class='info-label'>Year:</span><br>", input$year, "<br><br>",
      "<span class='info-label'>Children Tested:</span><br>",
      formatC(row$Total_Tests, format = "d", big.mark = ","), "<br><br>",
      "<span class='info-label'>Elevated Cases:</span><br>",
      formatC(row$Total_Elevated, format = "d", big.mark = ","), "<br><br>",
      "<span class='info-label'>Rate per 1,000:</span><br>",
